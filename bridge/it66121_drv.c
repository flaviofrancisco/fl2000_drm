 ...
 * (1280x720@60 preferred, 1920x1080@60 secondary) so the display still gets usable mode data.
 * 1920x1080 as the *preferred* timing was tried and reverted: drm_fbdev_dma_setup() sizes its
 * initial console framebuffer off the preferred mode, and on real hardware that ~8MB allocation
 * fails fbdev emulation with -ENOMEM (confirmed via dmesg: "fbdev: Failed to setup emulation
 * (ret=-12)"), leaving the display looking as if nothing is connected at all - console, DRM
 * clients, everything. 1920x1080 is still fully offered as the secondary detailed timing so it
 * can be selected manually; 1280x720's ~3.7MB buffer is what's proven to allocate reliably here.
 * Three of the eight standard-timing slots (bytes 38-43, otherwise unused/padding here) add
 * 1600x900@60 (16:9), 1280x1024@60 (5:4, the actual aspect of that resolution) and 1024x768@60
 * (4:3) as additional selectable steps.
 */
static const u8 it66121_fallback_edid[128] = {
	...
	0x99, 0x26, 0x0f, 0x50, 0x54, 0x28, 0x00, 0x00, 0xa9, 0xc0, 0x81, 0x80, 0x61, 0x40, 0x01,
	0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x1d, 0x00, 0x72, 0x51, 0xd0,
	0x1e, 0x20, 0xdc, 0x28, 0x55, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1e, 0x02, 0x3a, 0x80,
	0x18, 0x71, 0x38, 0x2d, 0x40, 0x58, 0x2c, 0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1e,
